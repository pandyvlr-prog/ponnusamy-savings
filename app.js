...
function renderDashboardMembersList(searchQuery = '') {
    ...
    filteredList.forEach((item, index) => {
        ...
        let statusBadgeHTML = item.currentMonthPaid 
            ? `<span class="pmc2-badge-paid"><i data-lucide="check" style="width:14px;height:14px;"></i> PAID${item.paymentMethodThisMonth === 'gpay' ? ' / G' : item.paymentMethodThisMonth === 'cash' ? ' / C' : ''}</span>`
            : item.paidAmount > 0 
                ? `<span class="pmc2-badge-pending" style="background:#fef3c7;color:#d97706;">PARTIAL</span>`
                : `<span class="pmc2-badge-pending">PENDING</span>`;

        let payoutBadgeHTML = item.hasTakenPayout
            ? `<span class="pmc2-badge-payout">₹${item.payoutVal.toLocaleString('en-IN')}${item.payoutMethod === 'cash' ? ' / C' : item.payoutMethod === 'gpay' ? ' / G' : ''}</span>`
            : `<span style="font-weight:700;color:var(--text-main);">--</span>`;

        let displayPaidDateRaw = item.displayPaidDate === 'N/A' || item.displayPaidDate === '--' ? '--' : item.displayPaidDate;
