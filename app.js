/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ...
   2. Toggles .kb-transitioning on <body> during resize events â€” the CSS rule
      for this class suppresses all transition/backdrop-filter, eliminating
      the blur ghost-frame flash.
   3. Global focusin â†’ smoothly scrolls focused inputs into view after 100ms
...
(function initKeyboardFix() {
    ...
    // Prefer visualViewport API (Chrome 61+, Safari 13+) â€” reports the area
...
/* â”€â”€â”€ Global Focus â†’ ScrollIntoView â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ...
   centers the focused input. Works across ALL modals/forms â€” not per-component.
   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
function setupEventListeners() {
    ...
    // Mobile quick-action bar Ã¢â‚¬â€ delegates to desktop button clicks
    ...
    const btnToggleToolbar = document.getElementById('btn-toggle-toolbar-m');
...
function _renderDashboard() {
    ...
    document.getElementById('stat-total-collected').textContent = isSyncing ? 'Syncing...' : 'â‚¹' + globalMetrics.totalCollected.toLocaleString('en-IN');
    
    const cashEl = document.getElementById('stat-summary-collected-cash');
    if (cashEl) cashEl.textContent = isSyncing ? '...' : 'â‚¹' + (globalMetrics.totalCollectedCash || 0).toLocaleString('en-IN');
    
    const gpayEl = document.getElementById('stat-summary-collected-gpay');
    if (gpayEl) gpayEl.textContent = isSyncing ? '...' : 'â‚¹' + (globalMetrics.totalCollectedGpay || 0).toLocaleString('en-IN');

    document.getElementById('stat-total-pending').textContent = isSyncing ? 'Syncing...' : 'â‚¹' + globalMetrics.totalPending.toLocaleString('en-IN');
            ...
            sortedGroups.forEach((group, index) => {
                ...
                card.innerHTML = `
                    <div class="group-card-header">
                        ...
                        <div class="group-card-amount" style="background: linear-gradient(135deg, #9333ea, #7e22ce); color: #ffffff; padding: 4px 10px; border-radius: 8px; border: none; font-weight: 900; letter-spacing: 0.5px; box-shadow: 0 4px 12px rgba(147,51,234,0.3);">â‚¹${Number(schemeAmount).toLocaleString('en-IN')}</div>
                    </div>
...
function renderDashboardMembersList(searchQuery = '') {
    ...
    // Keep isFuture items Ã¢â‚¬â€ they are valid months not yet tracked, and the user wants to see & mark them
    ...
    if (mPendingUnpaidEl) mPendingUnpaidEl.textContent = 'â‚¹' + syncPendingUnpaid.toLocaleString('en-IN');
    
    const mPendingPartialEl = document.getElementById('stat-summary-pending-partial');
    if (mPendingPartialEl) mPendingPartialEl.textContent = 'â‚¹' + syncPendingPartial.toLocaleString('en-IN');
    ...
    filteredList.forEach((item, index) => {
        ...
        let contactMenuHtml = '';
        if (item.isApplicable) {
            let contactBg = 'linear-gradient(135deg, #10b981, #047857)';
            let contactShadow = 'rgba(16, 185, 129, 0.4)';
            
            if (!item.currentMonthPaid) {
                contactBg = 'linear-gradient(135deg, #ef4444, #b91c1c)';
                contactShadow = 'rgba(239, 68, 68, 0.4)';
            }

            contactMenuHtml = `
                <div class="contact-action-wrapper" style="position: relative; display: inline-block;">
                    <button class="contact-action-btn" data-member-id="${item.member.id}" title="Contact" style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 8px; background: ${contactBg}; border: none; color: #ffffff; cursor: pointer; outline: none; box-shadow: 0 2px 5px ${contactShadow}; transition: transform 0.2s ease, box-shadow 0.2s ease;">
        ...
        row.innerHTML = `
            ...
            <div class="mobile-card-view dashboard-mobile-card" style="background:#f8f9fa;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);border:1px solid rgba(0,0,0,0.07);padding:0;">
                ...
                <!-- Name header -->
                <div class="mobile-card-header" style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border-bottom:1px solid rgba(0,0,0,0.04);cursor:pointer;background:linear-gradient(to right, rgba(255,255,255,0) 0%, rgba(255,255,255,0.5) 100%);">
                    <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0;">
                        <!-- Premium SNo Badge -->
                        <div style="background:linear-gradient(135deg, #eff6ff, #dbeafe);color:#1d4ed8;width:28px;height:28px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:0.95rem;flex-shrink:0;box-shadow:inset 0 1px 0 rgba(255,255,255,0.6), 0 2px 4px rgba(37,99,235,0.1);">
                            ${index + 1}
                        </div>
                        
                        <!-- Name and Amount Stack -->
                        <div style="display:flex;flex-direction:column;gap:3px;flex:1;min-width:0;">
                            <div style="display:flex;align-items:flex-start;gap:6px;flex-wrap:wrap;">
                                <h3 class="member-name" style="font-size:0.95rem;font-weight:900;color:#111827;margin:0;text-transform:uppercase;line-height:1.2;white-space:normal;word-break:break-word;">${item.member.name}</h3>
                                ${item.member.customerType === 'New' ? '<span style="background:linear-gradient(135deg, #f59e0b, #d97706);color:white;font-size:0.55rem;font-weight:800;padding:2px 6px;border-radius:4px;box-shadow:0 2px 4px rgba(217,119,6,0.2);margin-top:1px;">NEW</span>' : ''}
                            </div>
                            
                            <!-- Amount Row -->
                            <div style="display:flex;align-items:center;gap:4px;">
                                <span style="font-size:0.6rem; color:#6b7280; font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">${item.dueAmount === 0 ? 'PAID' : 'DUE'}</span>
                                <span style="font-size:0.95rem; font-weight:900; color:${item.dueAmount === 0 ? '#059669' : '#dc2626'}; display:flex; align-items:center; gap:2px;">
                                    &#8377;${(item.dueAmount + item.paidAmount).toLocaleString('en-IN')}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mobile-card-details" style="display:none;">
                    ...
                    <div>
                        ...
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:9px 14px;border-bottom:1px solid #f5f5f5;">
                            ...
                            <span style="font-size:0.95rem;font-weight:900;color:#dc2626;">&#8377;${item.dueAmount.toLocaleString('en-IN')}</span>
                        </div>
                        <!-- Paid Amount -->
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:9px 14px;border-bottom:1px solid #f5f5f5;">
                            <div style="display:flex;align-items:center;gap:8px;">
                            ...
                            <span style="font-size:0.95rem;font-weight:900;color:${item.paidAmount > 0 ? (item.currentMonthPaid ? '#059669' : '#d97706') : '#9ca3af'};">&#8377;${item.paidAmount.toLocaleString('en-IN')}</span>
                        </div>
                        <!-- Paid Date -->
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:9px 14px;border-bottom:1px solid #f5f5f5;">
                            <div style="display:flex;align-items:center;gap:8px;">
                ...
                </div> <!-- /mobile-card-details -->
            </div>
        `;

        // Mobile expand listener
        const expandHeader = row.querySelector('.mobile-card-header');
        const detailsContainer = row.querySelector('.mobile-card-details');
        if (expandHeader && detailsContainer) {
            expandHeader.addEventListener('click', (e) => {
                // Prevent toggle if clicked on contact buttons
                if (e.target.closest('.contact-action-wrapper')) {
                    return;
                }
                
                const isExpanded = detailsContainer.style.display === 'block';
                const btnIcon = row.querySelector('.mobile-expand-btn svg') || row.querySelector('.mobile-expand-btn i');
                const premiumArrow = row.querySelector('.premium-arrow');
                
                if (isExpanded) {
                    detailsContainer.style.display = 'none';
                    expandHeader.style.borderBottom = '1px solid #f0f0f0';
                    if (btnIcon) btnIcon.style.transform = 'rotate(0deg)';
                    if (premiumArrow) premiumArrow.style.transform = 'rotate(0deg)';
                } else {
                    detailsContainer.style.display = 'block';
                    expandHeader.style.borderBottom = 'none';
                    if (btnIcon) btnIcon.style.transform = 'rotate(180deg)';
                    if (premiumArrow) premiumArrow.style.transform = 'rotate(180deg)';
                }
            });
        }
...
function renderGroupDetails(groupId) {
    ...
    // Update Hero elements â€” start month green, end month red
    const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    const startMonth = group.startMonth !== undefined ? parseInt(group.startMonth) : new Date(group.createdAt).getMonth();
    const startYear = group.startYear !== undefined ? parseInt(group.startYear) : new Date(group.createdAt).getFullYear();
    const startDate = new Date(startYear, startMonth, 1);
...
function filterAndRenderMembers() {
    ...
    filteredMembers.forEach((member, index) => {
        ...
        let totalDue = group.installments && group.installments[group.currentMonth] !== undefined ? group.installments[group.currentMonth] : group.monthlyInstallment;
        let paymentData = member.payments[group.currentMonth] || {};
        let isPaid = paymentData.paid;
        let partialPaid = paymentData.partialPaid || 0;
        
        let dueAmountDisplay = 0;
        let paidAmountDisplay = 0;
        let paidAmountColor = '#059669'; // Green

        if (isPaid) {
            dueAmountDisplay = 0;
            paidAmountDisplay = totalDue;
            paidAmountColor = '#059669';
        } else if (partialPaid > 0) {
            dueAmountDisplay = totalDue - partialPaid;
            paidAmountDisplay = partialPaid;
            paidAmountColor = '#d97706';
        } else {
            dueAmountDisplay = totalDue;
            paidAmountDisplay = 0;
            paidAmountColor = '#6b7280';
        }

        let paidDateStr = (isPaid && paymentData.paidAt) ? new Date(paymentData.paidAt).toLocaleDateString('en-GB') : '---';
        ...
        let schemeLabelStr = `${schemeAmountStr} / ${group.duration}M`;

        let statusBadgeHTML = isPaid 
            ? `<span class="pmc2-badge-paid"><i data-lucide="check" style="width:14px;height:14px;"></i> PAID / ${paymentData.method ? paymentData.method.substring(0,1).toUpperCase() : 'G'}</span>`
            : `<span class="pmc2-badge-pending">DUE</span>`;

        let payoutBadgeHTML = (paymentData && paymentData.payoutClaimed)
            ? `<span class="pmc2-badge-payout">â‚¹${formatNumberIndian(paymentData.payoutClaimed === true ? (group.payouts && group.payouts[group.currentMonth] ? group.payouts[group.currentMonth] : group.chitAmount) : paymentData.payoutClaimed)} / ${paymentData.payoutMethod ? paymentData.payoutMethod.substring(0,1).toUpperCase() : 'C'}</span>`
        ...
        // Mobile layout â€” collapsed by default, expand on chevron click
        const mobileHTML = `
...
(function InstallmentCardsModule() {
    ...
    function saveData(data) {
        try {
            ...
            if(typeof showNotification === 'function') showNotification('Error saving cards: Storage full', 'error');
        }
    ...
    function parseValueLabel(label) {
        ...
        if (str.endsWith('k')) {
            ...
            multiplier = 100000;
            str = str.slice(0, -1);
        } else if (str.endsWith('lakh') || str.endsWith('lakhs')) {
            multiplier = 100000;
            str = str.replace(/lakhs?/, '');
        } else if (str.endsWith('cr') || str.endsWith('crore') || str.endsWith('crores')) {
            multiplier = 10000000;
            str = str.replace(/crores?|cr/, '');
    ...
    // Format inputs automatically if possible (e.g. 25k -> â‚¹25,000)
    ...
    function closeUploadWizard() {
        const backdrop = document.getElementById('ic-upload-wizard-backdrop');
        if (!backdrop || backdrop.style.display === 'none' || backdrop.classList.contains('closing')) return;
        backdrop.classList.add('closing');
        backdrop.querySelector('.ic-modal-premium').classList.add('closing');
        
        const finalize = () => {
            backdrop.style.display = 'none';
            backdrop.classList.remove('closing');
            backdrop.querySelector('.ic-modal-premium').classList.remove('closing');
            backdrop.removeEventListener('transitionend', finalize);
        };
        
        // Use transitionend with fallback
        backdrop.addEventListener('transitionend', finalize);
...
/* ============================================================
   LANDING PAGE SCROLL REVEAL LOGIC
   ============================================================ */
document.addEventListener('DOMContentLoaded', () => {
