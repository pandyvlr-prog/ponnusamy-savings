...
# 1. Update generateGlobalPdfReport signature
...
# 2. Update ROWS_PER_PAGE in generateGlobalPdfReport
$oldRows = "    const ROWS_PER_PAGE = 25;
    let chunkPagesHtml = \[\];"
$newRows = "    const ROWS_PER_PAGE = paperSize === 'custom' ? 12 : 25;
    let chunkPagesHtml = [];"
$content = [System.Text.RegularExpressions.Regex]::Replace($content, $oldRows, $newRows)

# 3. Update jsPDF generation logic in generateGlobalPdfReport
$oldPdfGen = "        const \{ jsPDF \} = window\.jspdf;
        const doc = new jsPDF\(\{ unit: 'mm', format: 'a4', orientation: 'portrait' \}\);
        const A4_WIDTH = 210;
        const A4_HEIGHT = 297;"
        
$newPdfGen = "        const { jsPDF } = window.jspdf;
...
$content = [System.Text.RegularExpressions.Regex]::Replace($content, $oldPdfGen, $newPdfGen)

# 4. Update A4_WIDTH and A4_HEIGHT references in generateGlobalPdfReport
