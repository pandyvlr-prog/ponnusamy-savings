$filePath = "c:\Users\ARAVIND\OneDrive\Desktop\ponnusamy savings\index.html"
$content = [System.IO.File]::ReadAllText($filePath)

$rupee = [char]0x20B9

$target = '                                <div style="display: flex; gap: 8px; margin-top: 10px; border-top: 1px solid rgba(48,209,88,0.2); padding-top: 8px;">
                                    <div style="flex: 1; text-align: center; background: rgba(234, 88, 12, 0.1); color: #ea580c; border: 1px solid rgba(234, 88, 12, 0.2); border-radius: 4px; font-size: 0.85rem; font-weight: 700; padding: 4px 2px;">
                                        CASH <br> <span id="stat-summary-collected-cash" style="font-size: 1.1rem; font-weight: 800;">__RUPEE__0</span>
                                    </div>
                                    <div style="flex: 1; text-align: center; background: rgba(147, 51, 234, 0.1); color: #9333ea; border: 1px solid rgba(147, 51, 234, 0.2); border-radius: 4px; font-size: 0.85rem; font-weight: 700; padding: 4px 2px;">
                                        GPAY <br> <span id="stat-summary-collected-gpay" style="font-size: 1.1rem; font-weight: 800;">__RUPEE__0</span>
                                    </div>
                                </div>'
$target = $target.Replace('__RUPEE__', $rupee)

$replacement = '                                <div style="display: flex; gap: 8px; margin-top: 10px; border-top: 1px solid rgba(48,209,88,0.2); padding-top: 8px;">
                                    <div style="flex: 1; text-align: center; background: rgba(234, 88, 12, 0.08); color: #ea580c; border: 1px solid rgba(234, 88, 12, 0.2); border-radius: 6px; font-size: 0.85rem; font-weight: 700; padding: 6px 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px;">
                                        <div style="display: flex; align-items: center; gap: 4px;">
                                            ...
                                            <svg width="18" height="12" viewBox="0 0 24 16" fill="none" xmlns="http://www.w3.org/2000/svg" style="vertical-align: middle;">
                                                ...
                                                <text x="12" y="10.8" font-family="sans-serif" font-size="8" font-weight="bold" fill="white" text-anchor="middle">__RUPEE__</text>
                                            </svg>
                                            CASH
                                        </div>
                                        <span id="stat-summary-collected-cash" style="font-size: 1.1rem; font-weight: 800;">__RUPEE__0</span>
                                    </div>
                                    <div style="flex: 1; text-align: center; background: rgba(66, 133, 244, 0.08); color: #4285F4; border: 1px solid rgba(66, 133, 244, 0.2); border-radius: 6px; font-size: 0.85rem; font-weight: 700; padding: 6px 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px;">
                                        ...
                                        <span id="stat-summary-collected-gpay" style="font-size: 1.1rem; font-weight: 800;">__RUPEE__0</span>
                                    </div>
                                </div>'
$replacement = $replacement.Replace('__RUPEE__', $rupee)

# Normalize content to LF and compare
...
if ($contentNormalized.Contains($targetNormalized)) {
    $newContent = $contentNormalized.Replace($targetNormalized, $replacementNormalized)
    # Write back matching input file's line ending style
    if ($content.Contains("`r`n")) {
        $newContent = $newContent -replace "`n", "`r`n"
    }
    [System.IO.File]::WriteAllText($filePath, $newContent)
    Write-Host "Success: index.html modified successfully."
} else {
    Write-Error "Error: Target content not found in index.html."
}
