...
$oldRegex = '<button class="btn btn-text-link" id="btn-quick-report-download"[\s\S]*?Download PDF[\s\S]*?</button>\s*<button class="btn btn-text-link" id="btn-quick-report-share"[\s\S]*?Share PDF[\s\S]*?</button>'
...
$content = [System.Text.RegularExpressions.Regex]::Replace($content, $oldRegex, $newHTML)

Set-Content -Path $indexPath -Value $content -Encoding UTF8
Write-Output "Patched via Regex"
